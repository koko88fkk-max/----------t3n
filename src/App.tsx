            ...
            {!authLoading && (
              user ? (
                <div className="flex items-center gap-2">
                  ...
                  >
                    <img src={user.photoURL || ''} alt="User" className="w-6 h-6 rounded-full object-cover border border-white/20" />
                ...
                <motion.button 
                  whileHover={{ scale: 1.05, y: -2, boxShadow: "0 0 20px rgba(88,101,242,0.4)" }}
                  whileTap={{ scale: 0.95 }}
                  onClick={onLogin}
                  className="h-10 px-4 rounded-full bg-[#5865F2] text-white border border-[#5865F2]/30 flex items-center justify-center hover:bg-[#4752C4] transition-all gap-2 shadow-lg"
                >
                ...
                >
                  ...
                  {!user && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={onLogin}
                      className="w-full mb-5 bg-[#5865F2] text-white rounded-2xl px-6 py-4 flex items-center justify-center gap-3 hover:bg-[#4752C4] transition-all font-bold shadow-lg"
                    >
...
function AdminDashboard({ onClose }: { onClose: () => void }) {
  ...
  const formatFullDate = (dateStr: string | number | undefined) => {
    if (!dateStr) return '-';
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {loading ? (
            ...
            <>
              ...
              {(activeTab === 'users' || activeTab === 'logins') && (
                      ...
                      <tbody>
                        {stats.users.filter((u:any) => u.verifiedKey && (u.email?.includes(userSearchTerm) || u.id?.includes(userSearchTerm.toLowerCase()) || getNumericId(u.id).includes(userSearchTerm))).map((u: any, i: number) => (
                          <tr key={u.id} className={`border-b border-white/5 hover:bg-white/5 transition-colors ${i % 2 === 0 ? 'bg-white/[0.02]' : ''}`}>
                            ...
                            <td className="px-4 py-3">
                              <span className="text-zinc-400 text-[10px] leading-tight block text-left" dir="ltr">{formatFullDate(u.verifiedAt)}</span>
                            </td>
                            <td className="px-4 py-3">
                      ...
                      <tbody>
                        {stats.users.filter((u:any) => u.email?.includes(userSearchTerm) || u.id?.includes(userSearchTerm.toLowerCase()) || getNumericId(u.id).includes(userSearchTerm)).sort((a:any,b:any)=>(b.lastLoginAt||'').localeCompare(a.lastLoginAt||'')).map((u: any, i: number) => (
                          <tr key={u.id} className={`border-b border-white/5 hover:bg-white/5 transition-colors ${i % 2 === 0 ? 'bg-white/[0.02]' : ''}`}>
                            ...
                            <td className="px-4 py-3">
                              <span className="text-zinc-400 text-[10px] leading-tight block text-left" dir="ltr">{formatFullDate(u.lastLoginAt)}</span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="text-zinc-400 text-[10px] leading-tight block text-left" dir="ltr">{formatFullDate(u.createdAt)}</span>
                            </td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-1">
                                {!u.banned ? (
